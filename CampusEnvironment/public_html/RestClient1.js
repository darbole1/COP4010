...
// Model for Student entity
    models.Student = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/StudentPortfolioWeb/webresources/edu.nsuni.entity.student/",
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8080/StudentPortfolioWeb/webresources/edu.nsuni.entity.student/';
            }
    ...
    // Collection class for Student entities
    models.StudentCollection = Backbone.Collection.extend({
        model: models.Student,
        url: "http://localhost:8080/StudentPortfolioWeb/webresources/edu.nsuni.entity.student/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.StudentCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-student-list-itemis template identifier for item
                        templateName: '#tpl-student-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.student = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.student,
                    // tpl-student-details is template identifier for chosen model element
                    templateName: '#tpl-student-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Student();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-student-details is a template identifier for chosen model element
                templateName: '#tpl-student-details',
